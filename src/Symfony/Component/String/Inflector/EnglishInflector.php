...
class EnglishInflector implements InflectorInterface
{
    ...
    {
        $pluralRev = $string->reverse();
        $lowerPluralRev = $pluralRev->lower();

        // Check if the word is one which is not inflected, return early if so
        if (\in_array($lowerPluralRev->toString(), self::$uninflected, true)) {
            return clone $string;
        }
        ...
        foreach (self::$pluralMap as $map) {
            ...
            while ($suffix[$j] === $lowerPluralRev->slice($j, 1)->toString()) {
                ...
                if ($j === $suffixLength) {
                    ...
                    if ($j < $pluralLength) {
                        $nextIsVocal = false !== strpos('aeiou', $lowerPluralRev->slice($j, 1)->toString());
                    ...
                    $newBase = $string->slice(0, $pluralLength - $suffixLength)->toString();
                    $newSuffix = $map[4];

                    // Check whether the first character in the plural suffix
                    // is uppercased. If yes, uppercase the first character in
                    // the singular suffix too
                    $firstUpper = ($s = $pluralRev->slice($j - 1, 1))->toString() === $s->upper()->toString();
                    ...
                    if (\is_array($newSuffix)) {
                        ...
                        foreach ($newSuffix as $newSuffixEntry) {
                            $singulars[] = new $class($newBase.($firstUpper ? ucfirst($newSuffixEntry) : $newSuffixEntry));
                        }

                        return $singulars;
                    }

                    return new $class($newBase.($firstUpper ? ucfirst($newSuffix) : $newSuffix));
                }
    ...
    {
        $singularRev = $string->reverse();
        $lowerSingularRev = $singularRev->lower();

        // Check if the word is one which is not inflected, return early if so
        if (\in_array($lowerSingularRev->toString(), self::$uninflected, true)) {
            return clone $string;
        }
        ...
        foreach (self::$singularMap as $map) {
            ...
            while ($suffix[$j] === $lowerSingularRev->slice($j, 1)->toString()) {
                // Let $j point to the next character
                ++$j;
                ...
                if ($j === $suffixLength) {
                    // Is there any character preceding the suffix in the plural string?
                    if ($j < $singularLength) {
                        $nextIsVocal = false !== strpos('aeiou', $lowerSingularRev->slice($j, 1)->toString());
                    ...
                    $newBase = $string->slice(0, $singularLength - $suffixLength)->toString();
                    $newSuffix = $map[4];

                    // Check whether the first character in the singular suffix
                    // is uppercased. If yes, uppercase the first character in
                    // the singular suffix too
                    $firstUpper = ($s = $singularRev->slice($j - 1, 1))->toString() === $s->upper()->toString();
                    ...
                    if (\is_array($newSuffix)) {
                        ...
                        foreach ($newSuffix as $newSuffixEntry) {
                            $plurals[] = new $class($newBase.($firstUpper ? ucfirst($newSuffixEntry) : $newSuffixEntry));
                        }
        ...
        return new $class($string->toString().'s');
    }
}
