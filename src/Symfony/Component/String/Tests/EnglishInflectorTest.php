...
final class EnglishInflectorTest extends TestCase
{
    private const STRING_CLASSES = [
        ByteString::class,
        CodePointString::class,
        UnicodeString::class,
    ];
     ...
     */
    public function testSingularize(string $stringClass, string $plural, $expected)
    {
        $result = (new EnglishInflector())->singularize(new $stringClass($plural));

        if (\is_string($expected) && \is_array($result)) {
            $this->fail("--- Expected\n`string`: ".$expected."\n+++ Actual\n`array`: ".implode(', ', $result));
        } elseif (\is_array($expected) && \is_string($result)) {
            $this->fail("--- Expected\n`array`: ".implode(', ', $expected)."\n+++ Actual\n`string`: ".$result);
        }

        $expected = !\is_array($result) ? new $stringClass($expected) : array_map(static function (string $expected) use ($stringClass): AbstractString {
            return new $stringClass($expected);
        }, $expected);

        $this->assertEquals($expected, $result);
    }

    public function singularizeProvider(): \Generator
    {
        foreach (self::STRING_CLASSES as $stringClass) {
            ...
            foreach ([
                ...
                [
                    'hippopotamuses',
                    ['hippopotamus', 'hippopotamuse', 'hippopotamusis']
                ],
                //hippopotami
                ['hoaxes', 'hoax'],
                ...
                yield [$stringClass, $plural, $expected];
            }
        }
    }
     ...
     */
    public function testPluralize(string $stringClass, string $singular, $expected)
    {
        $result = (new EnglishInflector())->pluralize(new $stringClass($singular));
    ...
    public function pluralizeProvider(): \Generator
    {
        foreach (self::STRING_CLASSES as $stringClass) {
            // see http://english-zone.com/spelling/plurals.html
            // see http://www.scribd.com/doc/3271143/List-of-100-Irregular-Plural-Nouns-in-English
            foreach ([
                ...
                ['SubTree', 'SubTrees'],
            ] as [$singular, $expected]) {
                yield [$stringClass, $singular, $expected];
            }
        }
    }
}
